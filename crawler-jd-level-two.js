...
function executeInsert(psurl, obj, conn) {
    ...
    var request = new Request(
        "insert into [LevelTwo] ([sid],[psurl],[surl],[sname],[map_x],[map_y],[map_info],[alias],[address],[pic_url],[full_url],[kaopu_remark],[more_desc]) " +
        " VALUES (@sid,@psurl,@surl,@sname,@map_x,@map_y,@map_info,@alias,@address,@pic_url,@full_url,@kaopu_remark,@more_desc)",
...
function executeQuery(conn) {
    var request = new Request("select [surl] from LevelOne where [statusflag]=0", function (err, rowCount) {
        if (err) {
            //console.log(err);
        } else {
            console.log(rowCount + ' rows');
        ...
        if (error) {
            ...
            if ($) {
                ...
                if (obj != null) {
                    ...
                    console.log("error parse. currpage: " + currpage + " surl " + filename);
                    currpage++;
                    setCrawlerQueueWhenPageChange();
                }
            } else {
